...
from generate import generate_decrypted
from shared import confirm, create_github_client, extract_ipa_metadata
from shared.config import config, get_app_config
...
def main():
    ...
    if confirm(default=True):
        release = github_client.rest.repos.create_release(
            owner=config.ipa_archive_repo.owner,
            repo=config.ipa_archive_repo.repo,
